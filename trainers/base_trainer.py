...
class BaseTrainer:
    ...
    def train(self):
        ...
        for cur_epoch in range(1, total_epoch_num+1):
            ...
            self.train_epoch()
            self.evaluate_epoch()
            self.eval_train, self.eval_validate = self.train_top1.avg.item(), self.eval_top1.avg.item()
            # printer
            consume_time = time.time()-epoch_start_time
            self.logger.log_printer.epoch_case_print(self.cur_epoch,
                                                     self.train_top1.avg.item(), self.eval_top1.avg.item(),
                                                     self.train_losses.avg,  self.eval_losses.avg,
                                                     consume_time)

            if self.eval_validate >= self.best_eval:
                # save model
                self.model.save()
                self.best_eval = self.eval_validate
            # logger
